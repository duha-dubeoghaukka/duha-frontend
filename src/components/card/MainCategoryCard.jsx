...
function MainCategoryCard({ item }) {
  const { title, route, description, background, none } = item;
    ...
    <Grid item xs={5} sm={5}>
      <Link to={route} className="relative">
        <Box className={`${background}`} p={"60px"}>
          {none ? null : (
            <>
              <div className="absolute w-full h-full top-0 left-0 w-full h-full bg-[#757575] opacity-70 rounded-lg brightness-75"></div>
